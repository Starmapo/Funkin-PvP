...
class ZipParser
{
	 ...
	 */
	public var centralDirectoryRecords:Map<String, CentralDirectoryFileHeader>;
	...
	function findEndOfCentralDirectoryRecord():Void
	{
		...
		endOfCentralDirectoryRecord = new EndOfCentralDirectoryRecord(fileHandle, -4);
	}
	...
	{
		centralDirectoryRecords = new Map();
		fileHandle.seek(endOfCentralDirectoryRecord.cdrOffset, SeekBegin);
		for (_ in 0...endOfCentralDirectoryRecord.cdrsTotal)
		{
			var cdh = new CentralDirectoryFileHeader(fileHandle);
			centralDirectoryRecords.set(cdh.fileName, cdh);
		}
	}
...
class LocalFileHeader extends Header
{
	...
	public function readData():Bytes
	{
		...
		if (bytesRead < compressedSize)
		{
			...
			return (compressionMethod == DEFLATE) ? ZipUtil.unzipBytes(bytesBuf.getBytes()) : bytesBuf.getBytes();
		}
		
		return (compressionMethod == DEFLATE) ? ZipUtil.unzipBytes(bytesToReturn) : bytesToReturn;
	}
